...
4) karst-native ABI AUSBAUEN (capability-/handle-basiert)
   ...
   - Syscalls im karst-native-Stil: handle-orientiert, Fehlercodes als eigener Typ (kein errno), keine ambient authority.
...
- Der Produktname kommt AUSSCHLIESSLICH aus kernel/src/kcore/branding.rs (gespeist aus Cargo-Metadaten via build.rs). Kein hartkodiertes "karst"/"Karstos" in kernel/src ausserhalb branding.rs — test.sh Schritt 14 prueft das.
...
(f) NAME NUR AUS branding.rs? `grep -rniE 'karst' kernel/src --include='*.rs' | grep -v branding.rs` muss leer sein. Und `./rename.sh` muss in einer Kopie unter /tmp nachweislich funktionieren.
