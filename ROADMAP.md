...
| 2.7 | **Einstellungen, Themen, Hintergrundbilder** — Farbschema als Datei, Bild laden, ein Programm dafuer. Fleissarbeit, sobald der Fensterserver steht. | **halb** (Runde UI). Das **Farbschema ist eine Datei**: `assets/theme` wird als `/etc/theme` in das Abbild gelegt, 31 benannte Farben, gelesen von ZWEI Seiten — `kernel/user/wlibc.fi` in Ring 3 (18 Widget-Farben) und `kernel/kmain.fi` im Kern (13 Farben des Fensterservers: Schreibtisch, Rahmen, Titelleiste, Terminalfenster). Die eingebauten Werte sind absichtlich die **alten**, damit die Gegenprobe echt ist: ohne die Datei meldet der Kern `wm: theme 0 / 13  vorgabe` und die Messung faellt zurueck (89 Farben, eine davon 69,8 %) statt 174 Farben, groesste 25,7 %. OFFEN bleiben: **Hintergrundbild** (der Schreibtisch ist ein Verlauf, kein Bild) und ein **Programm** zum Umstellen zur Laufzeit — heute muss man die Datei bearbeiten und neu starten. |
| 2.7b | **Optischer Feinschliff der Oberflaeche** — am 27.08.2026 von Justin beanstandet: die Bildschirmfotos sehen schlecht aus. Belegt und nicht geraten: **800x600**, **89 bis 178** verschiedene Farben je Bild, ein einziger Blauton (47,95,156) fuer jede Hervorhebung, und (30,42,56) auf **47 bis 49 %** der Flaeche. | **fertig** (Runde UI, Zweig `ui-politur`, Logbuch `docs/ROUNDUI.md`). Gemessen aus echten Bildschirmfotos ueber den QEMU-Monitor mit `tools/ui/messen.py`, Vorher/Nachher-Paare unter `.ui-shots/`. **Aufloesung** 800x600 → **1280x800** als Vorgabe (+113 % Bildpunkte); der Kern HANDELT sie aus statt sie zu behaupten (`fb.probe`: Faehigkeitsabfrage der Bochs-Schnittstelle, Bildspeicher, freie 2-MiB-Fensterplaetze, dann eine Leiter von 1920x1080 abwaerts). `fbnativ` gibt unter QEMU 1920x1080, `fbklein` holt 800x600 als Gegenprobe zurueck, `fbbig` bleibt 1024x768. **Farben je Bild** 89/160/178/146/129 → **174/204/222/201/193**. **Anteil der groessten Farbe** 47,0/48,5/35,9/49,4/49,4 % → **25,7/19,1/36,3/18,1/17,9 %** (der Dateimanager bleibt bei 36 %, weil dort eine Tabelle das Fenster fuellt — der einzige Wert, der sich nicht verbessert hat). **Schrift** 15/16 → **17/17 Punkt**; die Titelschrift misst im Bild **23 statt 14** Bildpunkte Tintenspanne. **Raster**: Titelleiste 22 → **32** (8×4), Rahmen 2 → **1** Bildpunkt (dafuer ein Schatten unten und rechts), Schliessfeld 14 → 20, Griff 12 → 16, Listenzeile 20 → 24, Bedienelemente **32** hoch, Raender 8/16 statt 6/8/12 gemischt. **Schreibtisch** ist ein senkrechter Verlauf statt einer Flaeche; **Titelschrift** senkrecht zentriert statt oben klebend; **Schliessfeld** neutral statt rot; **Fenster** (Dateimanager, Starter, Widget-Vorfuehrung, Terminal) rechnen Groesse und Ort aus der Bildschirmgroesse und stehen mittig. **Kontrast** aller Schrift-Paarungen nachgerechnet, alle ≥ 4,5:1 (WCAG AA), die beiden Nicht-Text-Werte ≥ 3:1. Farbwerte aus dem Entwurfsleitfaden (`slate`-Leiter + Tafel Nr. 81 „Developer Tool / IDE“), keiner erfunden. Drei aeltere Fehler, die die neue Aufloesung gefunden hat: `wlibc.start()` verweigerte ueber 800 Bildpunkten Breite den Dienst (**keine** Anwendung mit Oberflaeche waere gestartet), der Puffer fuer `/etc/theme` war mit 1024 Oktetten zu klein, und `tools/k15/run.sh` fuehrte Rahmen- und Titelmasse als eigene Zahlen. Laeufer: `tools/ui/run.sh` (5 Bilder, 22 Zusagen) plus Gegenprobe `--ohne-theme`. |
...
| 3.4 | **Grafikkarte** — heute nur der lineare Rahmenpuffer des Bootladers. Modussetzung und Beschleunigung waeren die naechste Stufe. | **Modussetzung teilweise** (Runde UI), Beschleunigung offen. `kernel/fb.fi` FRAGT die Karte jetzt, statt ihr einen Modus aufzudraengen: `caps_w`/`caps_h` ueber die Faehigkeitsabfrage der Bochs-Schnittstelle (`VBE_ENABLE = 2`, dann XRES/YRES lesen), `vram_kb` ueber Index 10 (64-KiB-Einheiten), und `probe` waehlt daraus die groesste Stufe einer Leiter (1920x1080 … 640x480), die in Bildspeicher UND in die freien 2-MiB-Fensterplaetze des Kernels passt — danach wird der Modus gesetzt und ZURUECKGELESEN. Neue Worte: `fbnativ` (groesster Modus, unter QEMU 1920x1080), `fbklein` (800x600). NICHT geloest: die **wirkliche native Aufloesung**. QEMU meldet als Faehigkeit 16000x12000 — eine Schranke, keine Bildschirmgroesse; **EDID wird nicht gelesen**, und der Weg ueber einen Lader mit Videoteil (Limine/GOP unter UEFI) wurde in dieser Runde nicht neu gemessen. Auf einem Rechner mit 1366x768-Panel wuerde `fbnativ` heute 1920x1080 nehmen und damit falsch liegen. Ebenfalls offen: jede Form von Beschleunigung — alles Malen ist weiter Software. |
