// ===========================================================================
//  VORLAGE — KEIN GEBAUTER CODE.
//
//  Diese Datei ist der EINZIGE Rest des Rust-Kernels, der am 26.08.2026
//  nicht geloescht wurde, und sie wird von nichts uebersetzt: es gibt in
//  diesem Repo kein `cargo`, kein `Cargo.toml` und kein `kernel/src`
//  mehr. Sie liegt hier als SPEZIFIKATION.
//
//  WARUM SIE STEHEN BLEIBT. `KERNELWECHSEL.md` § 4.2 hat sie als offenen
//  Punkt gefuehrt: Osum ist x86-64-only und hat keine Architekturgrenze,
//  obwohl Firn auch aarch64 kann. Sie ist NICHT nach Firn portiert
//  worden, und der Grund steht in einem Satz: das waere keine Portierung,
//  sondern eine Umbauarbeit an jedem Modul jenes Kernels — die x86-Details
//  stehen dort ueber `kmain.fi`, `proc.fi`, `mem.fi`, `apic.fi`,
//  `smp.fi`, `user.fi`, `guard.fi` verteilt, und eine Grenze zieht man
//  nicht, indem man Traits danebenlegt.
//
//  Was hier steht, ist also die ANFORDERUNG an die Runde, die diese
//  Grenze in Osum ziehen wird — und zwar in der Form, in der sie schon
//  einmal funktioniert hat: der Rust-Kernel dieses Repos HAT diese
//  Grenze gehabt, und ein Testschritt hat sie erzwungen, indem er
//  x86-Begriffe (`cr0`..`cr4`, `PML4`, `PTE`, `rdmsr`, `wrmsr`, `lgdt`,
//  `lidt`, `outb`, `inb`, `asm!`, `invlpg`, `iretq`, `swapgs`, `sysret`,
//  `STAR`, `LSTAR`, `SFMASK`, `SMEP`, `SMAP`) ausserhalb von `arch/`
//  verboten hat. Diese Regel ist der eigentliche Inhalt; die Traits
//  darunter sind ihre Ausformulierung.
//
//  WAS IN OSUM STATTDESSEN GILT: nichts dergleichen. Das steht so in
//  Osums README unter "Was ihm fehlt" und in ROADMAP.md dieses Repos als
//  eigener Punkt. Es verschwindet nicht stillschweigend.
//
//  DIE FUENF TYPEN, die weiter unten benutzt werden, standen in
//  `kernel/src/kcore/mem.rs` (geloescht, in der Historie: `git show
//  91182a0:kernel/src/kcore/mem.rs`). Zum Mitlesen hier ihre Kerne:
//
//      pub struct PhysAddr(pub u64);
//      pub struct VirtAddr(pub u64);
//      pub struct MapFlags(pub u32);   // READ WRITE EXEC USER NO_CACHE GLOBAL
//      pub enum MapError { OutOfFrames, AlreadyMapped, NotMapped,
//                          Misaligned, ParentHugePage }
//      pub trait FrameAllocator {
//          fn alloc_frame(&mut self) -> Option<PhysAddr>;
//          fn free_frame(&mut self, frame: PhysAddr);
//      }
//
//  Kein einziges x86-Bit kommt darin vor. Das ist der Punkt.
// ===========================================================================
